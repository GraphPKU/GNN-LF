# copied from https://github.com/torchmd/torchmd-net/blob/main/torchmdnet/models/utils.py
# basis to build edge feature 
import torch
...
class SincRadialBasis(nn.Module):
    def __init__(self, n_rbf, rbound_upper, rbf_trainable=False, **kwargs):
        super().__init__()
        if rbf_trainable:
            self.register_parameter("n", nn.parameter.Parameter(torch.arange(
                1, n_rbf + 1, dtype=torch.float).unsqueeze(0)/rbound_upper))
        else:
...
class ExpNormalSmearing(nn.Module):
    def __init__(self, num_rbf, rbound_upper, rbound_lower, rbf_trainable=False, **kwargs):
        super().__init__()
        self.rbound_upper = rbound_upper
        self.rbound_lower = rbound_lower
        self.num_rbf = num_rbf
